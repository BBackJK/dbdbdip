...
<div>
  ...
  >
    ...
    <v-card-text>
      ...
      <v-btn x-small :disabled="orderQuantity === 1" @click="onDownQuantity"><v-icon>mdi-minus</v-icon></v-btn>
          {{orderQuantity}}
    ...
    <v-card-actions>
      ...
      <v-spacer></v-spacer>
        ...
        @click="show = !show"
      >
      Description <v-icon>{{ show ? 'mdi-chevron-up' : 'mdi-chevron-down' }}</v-icon>
      </v-btn>
...
export default {
  ...
  methods: {
    ...
    onOrder(item) {
      ...
      this.$store.dispatch('order/pushOrderData', item);
